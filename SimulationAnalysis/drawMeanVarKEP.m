...
for j =1:1:variable_size
    for i = 1:1:output_size
        filename = sprintf('ResultKEP//%f_%f_%f_%f_%f_%f_%f_%f.mat',[(K_coefficient(i)),...
            k_on_max(j),k_on,k_off,mu_max,mu(j),delta,friction_coef]);
        load(filename);
...
% 使用 plot 的矩阵输入创建多行
plot1 = plot(K_coefficient,mRNA_theor_mean,'LineWidth',1,'Parent',axes1);
set(plot1(2),...
...
% 创建 scatter
scatter(K_coefficient,mRNA_mean(2,:),...
...
% 创建 scatter
scatter(K_coefficient,mRNA_mean(1,:),...
...
% 创建 scatter
scatter(K_coefficient,mRNA_mean(3,:),...
...
% 使用 plot 的矩阵输入创建多行
plot1 = plot(K_coefficient,mRNA_theor_CV,'LineWidth',1,'Parent',axes1);
set(plot1(2),...
    'Color',[0.0392156876623631 0.658823549747467 0.250980406999588]);
set(plot1(1),...
    'Color',[0.952941179275513 0.149019613862038 0.258823543787003]);
...
if simulated_on == true
% 创建 scatter
scatter(K_coefficient,mRNA_CV(2,:),...
    'MarkerEdgeColor',[0.0392156876623631 0.658823549747467 0.250980406999588],...
    'LineWidth',1,...
    'MarkerFaceColor',[0.894117653369904 0.941176474094391 0.901960790157318]);

% 创建 scatter
scatter(K_coefficient,mRNA_CV(1,:),...
    'MarkerEdgeColor',[0.952941179275513 0.149019613862038 0.258823543787003],...
    'LineWidth',1,...
    'MarkerFaceColor',[0.960784316062927 0.921568632125854 0.921568632125854]);

% 创建 scatter
scatter(K_coefficient,mRNA_CV(3,:),...
    'MarkerEdgeColor',[0 0.447058826684952 0.721568644046783],...
    'LineWidth',1,...
    'MarkerFaceColor',[0.87058824300766 0.921568632125854 0.980392158031464]);
end
